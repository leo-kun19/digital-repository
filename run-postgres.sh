...
ENV_FILE="${ROOT_DIR}/config/postgres.env"
EXAMPLE_FILE="${ROOT_DIR}/config/postgres.env.example"
...
require_postgres_env() {
  local missing=()
  local placeholders=()
  local vars=(DB_URL DB_USER DB_PASS)
  ...
  if (( ${#missing[@]} > 0 || ${#placeholders[@]} > 0 )); then
    echo "PostgreSQL configuration is incomplete."
    if (( ${#missing[@]} > 0 )); then
      echo "Missing vars: ${missing[*]}"
    fi
    if (( ${#placeholders[@]} > 0 )); then
    ...
    echo "Update ${ENV_FILE} before running."
    exit 1
  fi
}
